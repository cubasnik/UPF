...
```text
demo establish
demo downlink-tool 1200
show status json
show n6-buffer json
exit
```

`demo downlink-tool [bytes] [imsi pdu] [preset=<name>] [dnn=<name>] [profile=<ipv4|ipv6|ethernet>]` сам запускает `n6_traffic_tool` с коротким burst и затем выполняет реальный downlink для уже установленной demo-сессии. Для полного one-shot сценария в REPL доступен `demo full-tool [bytes] [imsi pdu] [preset=<name>] [dnn=<name>] [profile=<ipv4|ipv6|ethernet>]`.

По умолчанию REPL использует встроенную demo-сессию `250200123456789/10`, но теперь любую demo-команду можно направить в конкретную сессию. Примеры:

```text
demo establish 250200123450001 21
demo uplink 1500 250200123450001 21
demo downlink-tool 1200 250200123450001 21
demo release 250200123450001 21
```

Для быстрого переключения N6 traffic-profile без изменения `n6_default_protocol` используйте named options:

```text
demo establish 250200123450002 22 dnn=ims profile=ipv6
demo full-tool 1200 250200123450002 22 dnn=ims profile=ipv6
demo downlink-tool 900 250200123450003 23 dnn=enterprise profile=ethernet
```
...
```text
demo validate 250200123450002 22 preset=ims-ipv6
demo validate 1440 250200123450002 22 preset=ims-ipv6
demo validate 250200123450002 22 preset=ims-ipv6 json
demo validate 1440 250200123450002 22 preset=ims-ipv6 json
demo validate tool-cmd 1440 250200123450002 22 preset=ims-ipv6
demo validate tool-cmd 1440 250200123450002 22 preset=ims-ipv6 json
demo validate 1440 250200123450002 22 compare=ims-ipv4,ims-ipv6
demo validate tool-cmd 1440 250200123450002 22 compare=ims-ipv4,ims-ipv6 json
...
demo establish 250200123450002 22 preset=ims-ipv6
demo full-tool 1200 250200123450002 22 preset=ims-ipv6
demo downlink-tool 900 250200123450003 23 preset=enterprise-ethernet
show demo-presets
show demo-presets json
show demo-matrix
show demo-matrix json
show demo-matrix 250200123450002 22 1440
show demo-matrix 250200123450002 22 1440 json
show demo-matrix 250200123450002 22 1440 tool-cmd
show demo-matrix 250200123450002 22 1440 tool-cmd json
show demo-matrix 250200123450002 22 1440 preset=ims-ipv6
show demo-matrix 250200123450002 22 1440 preset=ims-ipv6 tool-cmd json
show demo-matrix 250200123450002 22 1440 compare=ims-ipv4,ims-ipv6
show demo-matrix 250200123450002 22 1440 compare=ims-ipv4,ims-ipv6 json
show demo-compare 250200123450002 22 1440 compare=ims-ipv4,ims-ipv6
show demo-compare 250200123450002 22 1440 compare=ims-ipv4,ims-ipv6 tool-cmd json
```

Legacy shortcut `profile=ims-ipv6` тоже остаётся валидным, но основной рекомендуемый вариант теперь `preset=...`.

`show demo-presets` печатает доступные preset-ы и их разворот в `dnn=... profile=...`, чтобы сценарий можно было подобрать прямо из REPL без просмотра README.

`show demo-presets json` возвращает schema-tagged JSON (`upf.demo-presets.v1`) с массивом `presets`, где каждый элемент содержит `name`, `dnn` и `profile`.

`show demo-matrix` строит полный derived preview для всех preset-ов на дефолтной demo-сессии (`250200123456789` / `10`, `1200` bytes), чтобы быстро сравнить `teid`, UE addresses и profile-specific поля между `internet`, `ims` и `enterprise`.

Если передать `imsi pdu` и опционально `bytes`, матрица будет построена для этой сессии, а не для дефолтной demo-пары.
...
`show demo-matrix json` возвращает schema-tagged JSON (`upf.demo-matrix.v1`) с top-level `imsi`, `pdu`, `bytes` и массивом `entries` по всем preset-ам.

Режим `show demo-matrix ... tool-cmd` разворачивает ту же матрицу, но для каждой preset-строки дополнительно показывает resolved `tool_path`, `config_path` и итоговую shell-команду запуска `n6_traffic_tool`. Для JSON-режима используется отдельная схема `upf.demo-matrix-tool-command.v1`.

`show demo-compare` даёт такой же preview-only compare output, но в отдельной локальной команде без привязки к `demo validate` или `demo compare`. Он требует `compare=<preset1,preset2>`, принимает тот же набор `imsi pdu [bytes]`, а также поддерживает `tool-cmd` и `json` с теми же схемами `upf.demo-compare.v1` и `upf.demo-compare-tool-command.v1`.

`demo validate ...` не создает PFCP/N6/N3 трафик, а только показывает итоговый resolved target после применения `preset=...`, `dnn=...` и `profile=...`: bytes preview, IMSI, PDU, DNN, protocol profile, TEID, UE addresses/MAC и `request_id`.
...
Суффикс `json` переключает этот же вывод в schema-tagged JSON (`upf.demo-target.v1`) с полями `bytes`, `imsi`, `pdu`, `dnn`, `profile`, `teid`, `ue_ipv4`, `ue_ipv6`, `ue_mac` и `request_id`.

Режим `demo validate tool-cmd ...` использует тот же target/bytes preview, но дополнительно показывает resolved `tool_path`, `config_path` и итоговую shell-команду запуска `n6_traffic_tool`. Для JSON-режима используется отдельная схема `upf.demo-tool-command.v1`.

Опция `compare=<preset1,preset2>` переключает `demo validate` в режим `demo-compare`: вместо одного resolved target выводятся две compare-entries для указанных preset-ов на одной IMSI/PDU/bytes-паре. Этот режим взаимоисключающий с `preset=`, `dnn=` и `profile=`. Для `tool-cmd json` используется схема `upf.demo-compare-tool-command.v1`.

Команда `demo compare ...` является коротким alias для этого compare-preview режима и использует тот же вывод, те же JSON-схемы и те же ограничения аргументов.
...
Ограничения `demo compare` такие же строгие:
...
```text
demo compare 1440 250200123450002 22 preset=ims-ipv6 compare=ims-ipv4,ims-ipv6
# ERR: compare cannot be combined with preset, dnn, or profile

demo compare 1440 250200123450002 22 compare=ims-ipv6
# ERR: compare must be preset1,preset2
```

Одноразовый demo-запуск без REPL:
...
Для одноразового demo-прогона без REPL:
...
```text
demo validate 250200123450001 21 preset=ims-ipv6
demo validate 1440 250200123450001 21 preset=ims-ipv6
demo validate 250200123450001 21 preset=ims-ipv6 json
demo validate 1440 250200123450001 21 preset=ims-ipv6 json
demo validate tool-cmd 1440 250200123450001 21 preset=ims-ipv6
demo validate tool-cmd 1440 250200123450001 21 preset=ims-ipv6 json
demo validate 1440 250200123450001 21 compare=ims-ipv4,ims-ipv6
demo validate tool-cmd 1440 250200123450001 21 compare=ims-ipv4,ims-ipv6 json
demo establish 250200123450001 21 preset=ims-ipv6
demo downlink-tool 1200 250200123450001 21 preset=ims-ipv6
show demo-presets
show demo-presets json
show demo-matrix
show demo-matrix json
show demo-matrix 250200123450001 21 1440
show demo-matrix 250200123450001 21 1440 json
show demo-matrix 250200123450001 21 1440 tool-cmd
show demo-matrix 250200123450001 21 1440 tool-cmd json
show demo-matrix 250200123450001 21 1440 preset=ims-ipv6
show demo-matrix 250200123450001 21 1440 preset=ims-ipv6 tool-cmd json
show demo-matrix 250200123450001 21 1440 compare=ims-ipv4,ims-ipv6
show demo-matrix 250200123450001 21 1440 compare=ims-ipv4,ims-ipv6 json
show status json
show n6-buffer json
exit
```

Недопустимые варианты для `show demo-compare`:

```text
show demo-compare 250200123450002 22 1440 preset=ims-ipv6 compare=ims-ipv4,ims-ipv6
# ERR: compare cannot be combined with preset, dnn, or profile

show demo-compare 250200123450002 22 1440 compare=ims-ipv6
# ERR: compare must be preset1,preset2
```

`demo full` в PR-2.2 сценарии тоже зависит от внешнего N6 packet. Если `n6_traffic_tool` не запущен и никакой другой peer не шлёт datagram в `n6_bind`, downlink завершится `timeout`.
...
По умолчанию tool ищет `config/upf-config.yaml` как из корня репозитория, так и из каталога `build`, шлёт пакет в `n6_bind` и использует demo-значения той же PDU-сессии, что и `upf`. При необходимости можно переопределить `--endpoint`, `--imsi`, `--pdu`, `--protocol`, `--src`, `--dst`, `--bytes`, `--count` и `--interval-ms`.
